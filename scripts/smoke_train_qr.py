# Autor: Massanori
# Data: 19/05/2026
# Descrição: Smoke test do Grupo B (QR) para validar o pipeline antes do
#            treino completo. Mesmo padrao do smoke_train_resm.py, mas com:
#            QuantileRegressionModule + qr_loss + alpha=0.10. Criterio:
#            razao_final/inicial < 0.9. Roda em ~3-10 min no T4.


"""Smoke test do Grupo B (QR) para validar o pipeline de treino.

Replica a estrutura do smoke_train_resm.py mas usa a interface unificada
(D3) trocando apenas o modulo e a loss:
    - modulo: QuantileRegressionModule (2 U-Nets, ~15.5M params)
    - loss: qr_loss com alpha=0.10

Gate de sanidade antes do treino completo. Se loss nao diminui em 500
iters, ha bug — investigar antes de gastar 12h GPU.

Roda com:
    python scripts/smoke_train_qr.py --device cuda \\
        --recons-dir /kaggle/input/datasets/<user>/tcc-mri-recons-varnet-brain-4x \\
        --run-dir /kaggle/working/runs/group_b_smoke
"""
...
logger = logging.getLogger('smoke_train_qr')


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description='Smoke test do Grupo B (QR) — 500 iters.'
    )
    parser.add_argument('--recons-dir', type=Path, default=None)
    parser.add_argument('--device', default='auto', choices=['auto', 'cpu', 'cuda'])
    parser.add_argument('--n-iters', type=int, default=500)
    parser.add_argument('--run-dir', type=Path, default=ROOT / 'checkpoints' / 'smoke_qr')
    parser.add_argument('--keep-run', action='store_true')
    parser.add_argument('--config', type=Path, default=ROOT / 'configs' / 'training_base.json')
    parser.add_argument(
        '--alpha', type=float, default=DEFAULT_ALPHA,
        help=f'Nivel do quantile interval. Default {DEFAULT_ALPHA}.',
    )
...
def main() -> int:
    ...
    logger.info(f'Config carregado: {args.config.name}, alpha={args.alpha}')
    ...
    # Criterio de sanidade: razao_final/inicial < 0.9
    ...
    if len(train_losses) < 4:
        logger.error(f'Apenas {len(train_losses)} pontos de loss — log_every alto demais?')
        return 3
    ...
    logger.info('=' * 60)
    logger.info(f'Resultado do smoke test (Grupo B):')
